...
private:
  ...
  if(p == NULL){
    ...
    while(p->value != key_value){
      //std::cout << "???";
      if(p->value > key_value){
        //std::cout << "Going left: " << p->value << "\n";
        p = p->left;
      }

      else if(p->value < key_value){
        //std::cout << "Going right: " << p->value << "\n";
        p = p->right;
      }
  ...
  int erase(T const& key_value, TreeNode<T>* &p) {   return erase(key_value, root_); /* Implemented in Lecture 18 */  }
