...
void read_grid(istream& istr, vector<vector<int> > & blocked_grid,
  ...
  vector<int> one_row_of_ys(max_y+1, 2); // default is 2
  vector<vector<int> > empty_grid(max_x+1, one_row_of_ys);
  ...
  for (p = blocked_points.begin(); p != blocked_points.end(); ++p) {
    blocked_grid[p->x][p->y] = 1; // blocked is 1
  }
}
...
void print_grid(vector<vector<int> > const& blocked_grid, int start_x, int start_y) {
  ...
  for (int y=0; y<blocked_grid[0].size(); ++y) {
	...
	cout << " S";
      else if (blocked_grid[x][y] == 1)
	cout << " X";
		else if (blocked_grid[x][y] == 2){
			cout << " .";
		}
      else 
	cout << " $";
...
void print_grid2(vector<vector<int> > const& blocked_grid) {
  ...
  int grid(int x, int y, vector<vector<int> > blocked_grid ){
	int paths = 0;
	vector<vector<int> > blocked_grid2(blocked_grid);

	if (x == 0 && y == 0){
		print_grid2(blocked_grid2);
		return -1;
		//recurse here with x-1 or something? 
	} 
	...
	if (x !=0){ // if (x != 0  && !blocked_grid[x-1][y]) // have to do the !xyz as second conditional because if x =0 and you check b_grid of [x-1][y], it will seg fault because there is no -1 position on the grid
		blocked_grid2[x][y] = 3;
		int temp = grid(x-1, y, blocked_grid2);
		if (temp == -1) return -1;
		else paths += temp;
	}

	if (y !=0){ // if (y != 0 && !blocked_grid[x][y-1])
		blocked_grid2[x][y] = 3;
		int temp = grid(x, y-1, blocked_grid2);
		if (temp == -1) return -1;
		else paths += temp;
	}
	return paths;
...
int paths(int x, int y, const vector<vector<int> > & blocked, vector<int>& ptx, vector<int>& pty)
{
  ptx.push_back(x);
  pty.push_back(y);
  if(x == 0 && y == 0){
  ...
  if(x==0 || y==0){
    if(blocked[x][y] == 2){
      if(x==0){
        return paths(x, y-1, blocked, ptx, pty);
      }
      else{
        return paths(x-1, y, blocked, ptx, pty);
      }
    }
    else
    {
  ...
  {
    return paths(x-1, y, blocked, ptx, pty) + paths(x, y-1, blocked, ptx, pty);
  }
}
...
int main(int argc, char* argv[]) {
  ...
  vector<vector<int> > blocked_grid;
...
std::cout << grid(start_x,start_y, blocked_grid) << std::endl;


vector<int> pointsx;
vector<int> pointsy;
paths(start_x, start_y, blocked_grid, pointsx, pointsy) << '\n';
