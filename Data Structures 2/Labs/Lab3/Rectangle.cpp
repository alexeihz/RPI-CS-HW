...
bool Rectangle::is_point_within(const Point2D& p) const {
  return
    (p.x() <= m_upper_right.x() && p.y() <= m_upper_right.y()) && (p.x() >= m_lower_left.x() && p.y() >= m_lower_left.y());
}

bool Rectangle::add_point(const Point2D& p){
	if(is_point_within(p)==false){
		return false;
	}
	else{
		for(unsigned i = 0; i<m_points_contained.size(); i++){
...
void print_rectangle(const Rectangle& r1) {
  ...
  std::cout << "Rectangle: " 
            << r1.lower_right_corner().x() << "," << r1.lower_left_corner().y() << "--"
