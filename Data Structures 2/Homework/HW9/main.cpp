...
// class hash {
// public:
  // unsigned int operator() ( const Image<int>& img, int seed, Point& coordnate) const {
    // unsigned int hash = 1315423911;
    // for(unsigned int i = 0; i < coordnate.x + seed; i++){
		// for(unsigned int j = 0; j < coordnate.y + seed; j++){
			// hash ^= ((hash << 5) + img.getPixel(i,j) + (hash >> 2));
		// }
    // return hash;
  // }     
// };
...
void SimpleCompare(const Image<int>& a, const Image<int>& b, 
  ...
  // edge of one of the images or a mismatched pixel.
    ...
    int i2 = offset_b.x + 1;
	int	j2 = offset_b.y;
	while(i < a.Width() - seed && i2 < b.Width() - seed && isMatch){
        bool match = true;
        // Check for small seed match
		int x = 0;
		while(x < seed && match){
    ...
    j = offset_a.y + 1;
    j2 = offset_b.y + 1;
    while (j < a.Height() - seed && j2 < b.Height() - seed && isMatch) {
        bool match = true;
        i = offset_a.x;
        i2 = offset_b.x;
        while (i < offset_a.x + width - seed && match) {
            // Check for small seed match
			int x = 0;
            while (x < seed && match) {
				int y = 0;
