...
double ImprovedDistanceFieldMethod(Image<Color> &input, Image<DistancePixel> &distance_image) {
  ...
  // return the maximum distance value
  double answer = 0;
  std::set<DistancePixel> black_pixels;
  // loop over the pixels in the input image
	for (int i = 0; i < w; i++)  {
		for (int j = 0; j < h; j++) {
			const Color& c = input.GetPixel(i,j); 
			if(c.isBlack()){
				black_pixels.insert(distance_image.GetPixel(i,j));
			}  
		}
	}
  ...
  for (int i = 0; i < w; i++)  {
    for (int j = 0; j < h; j++) {
      double closest = -1;      
		...
		// skip all pixels that are not black
		if (!c.isBlack()) continue;
		std::set<DistancePixel>:: iterator itr;
		for (itr = black_pixels.begin(); itr!=black_pixels.end(); ++itr) {
			// calculate the distance between the two pixels
			double distance = sqrt((i-(*itr).getX())*(i-(*itr).getX()) + (j-(*itr).getY())*(j-(*itr).getY()));
			// store the closest distance to a black pixel
			if (closest < 0 || distance < closest) {
				closest = distance;
			}
			...
			if(c.isBlack()){
				...
				for(int k = 0; k < 8; k++){
					if(i == 0){
						...
						if(input.GetPixel(i-1,j).isBlack()==false){
