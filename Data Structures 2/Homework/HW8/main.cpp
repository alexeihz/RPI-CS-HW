...
void StudentTests() {
  ...
  //
   ...
   bidirectional_map<int,std::string>::key_iterator ki; 
   bidirectional_map<int,std::string>::value_iterator vi; 
  std::vector<std::string> values;
  values.push_back("carrot");
  values.push_back("banana");
  values.push_back("date");
  values.push_back("fig");
  values.push_back("eggplant");
  ...
  bidirectional_map<int,std::string> bdmap;
  bidirectional_map<int,std::string> bdmap2;
  for (int i = 0; i < 6; i++) {
	bdmap.insert(std::make_pair(keys[i],values[i])); 
  }
	...
	std::cout << values[0] <<" = " << keys[0] << std::endl;
	std::cout << keys[0] <<" = " << values[0] << std::endl;
	
	
	ki = bdmap.find(keys[0]);
	std::cout << (*ki).first <<" = " << keys[0] << std::endl;
	std::cout << (*ki).second <<" = " << values[0] << std::endl;
