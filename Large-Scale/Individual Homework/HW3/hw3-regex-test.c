...
bool backslash_check(char * input_line, const char * regex, char * matched, int * count_check){
    ...
    if(regex[regex_index] == '\\' || regex[regex_index] == '.' || regex[regex_index] == '^' || regex[regex_index] == '$' || regex[regex_index] == '?'
        || regex[regex_index] == '+' || regex[regex_index] == '*' || regex[regex_index] == '{' || regex[regex_index] == '}' || regex[regex_index] == '['
        || regex[regex_index] == ']' || regex[regex_index] == '(' || regex[regex_index] == ')'){
...
void bracket_check(char * input_line, const char * regex, char * matched, int * count_check){
    ...
    while(count_check[0] < strlen(input_line)){
        ...
        while (num < j){
            ...
            else if(oof[0] == '\\' && oof[1] == 'S' && not_space_check(input_line, oof, matched2, count_check) && not == false){
                matched[len] = matched2[0];
                len++;
                if(one == true){
                    two = true;
            ...
            else if(oof[0] == '\\' && oof[1] == 'S' && space_check(input_line, oof, matched2, count_check) && not == true){
                matched[len] = matched2[0];
                len++;
                if(one == true){
                    two = true;
...
int regex_match( const char * filename, const char * regex, char *** matches, int trim_to_match, char *** groups, int * captured_groups ){
    ...
    while (fgets(input_line, 256, input) != NULL){    
        ...
        for(count_check[0] = 0; count_check[0] < input_len; count_check[0]++) {
            ...
            if (count_check[1] == strlen(regex) && count_check[3] == true){
                ...
                if(trim_to_match == 1){
                    //matches[match_size][0] = matched;
                    printf("%s\n",matched);
                }
                else{
                    //matches[match_size][0] = input_line;
                    printf("%s",input_line);
                }
                match_size++;
                empty(matched);
                 ...
                 char *** groups, int * captured_groups )
{
  *matches = calloc( 3, sizeof( char * ) );
  (*matches)[0] = calloc( 10, sizeof( char ) );
  (*matches)[1] = calloc( 10, sizeof( char ) );
  (*matches)[2] = calloc( 10, sizeof( char ) );

  strcpy( (*matches)[0], "test 1" );
  strcpy( (*matches)[1], "test 2" );
  strcpy( (*matches)[2], "test 3" );

  *groups = calloc( 3, sizeof( char * ) );
  (*groups)[1] = calloc( 10, sizeof( char ) );
  (*groups)[2] = calloc( 10, sizeof( char ) );
