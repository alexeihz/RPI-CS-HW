...
void AddPrevOddPointers(Node* head, Node*& tail){
	//std::cerr << "I have not yet written AddPrevOddPointers (recursive)" << std::endl;
	if(head!=NULL && head->next_!=NULL && head->next_->next_ != NULL){
		head->next_->next_->prev_odd_ = head;
		tail = head->next_->next_;
...
void AddPrevOddPointersIterative(Node* head, Node*& tail){
	//std::cerr << "I have not yet written AddPrevOddPointers (iterative)" << std::endl;
	if(head == NULL){
		head = tail = NULL;
	}
	else if(head -> next_ == NULL || head -> next_ -> next_ == NULL){
		tail = NULL;
		head -> prev_odd_ = NULL;
	}
	else{
