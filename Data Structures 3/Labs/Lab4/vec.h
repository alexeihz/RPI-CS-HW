...
template <class T> typename Vec<T>::size_type remove_matching_elements(Vec<T> &vec, const T &t){
	typename Vec<T>::size_type num = 0;
	for(typename Vec<T>::iterator itr = vec.begin(); itr!=vec.end(); ++itr){
		if(*itr == t){
			vec.erase(itr);
			itr--;
			num++;
			vec.resize(vec.size(), 0);
		}
	}
	return num;	
}
