...
int panjm(int jhkg, int ozns) {
  ...
  rspic = std::modf(sqrt(aiwwyc), wvdst);
  if(rspic == 0){
    double temp = *wvdst;
	delete wvdst;
	return (int) temp;
  ...
  float nani = jhkg*jhkg - ozns*ozns ;
  rspic = std::modf(sqrt(nani), wvdst);
  if(rspic == 0){
	double temp = *wvdst;
	delete wvdst;
	return (int) temp;
...
int dambeq() {
  ...
  int qdcgba = ((ancy / ixgn) +5)/iglzqy; //  3 
  ...
  float djher = float(iglzqy) / float(ancy); // 0.1 
...
int nw_jtj() {
  ...
  assert(ljdsq[0][0] == 0);
  assert(ljdsq[0][hmgo-1] == 0);
  assert(ljdsq[hmgo-1][0] == 0);
  assert(ljdsq[hmgo-1][hmgo-1] == 0);
  ...
  int** fawbg = ljdsq;
 // int** aadvzu = bdfbc;
  for(int gwcnpy = 0; gwcnpy < hmgo; ++gwcnpy, ++fawbg) {
    int* kvctc = *fawbg;
    for(int wvp_u = 0; wvp_u < hmgo; ++wvp_u) {
      int vhnuig = *kvctc;
