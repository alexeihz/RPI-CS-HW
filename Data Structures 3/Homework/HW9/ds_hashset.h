...
public:
  ...
  std::pair< iterator, bool > insert(KeyType const& key, float occupancy) {
  ...
  iterator find(const KeyType& key) {
	...
	for(hash_list_itr p = m_table[index].begin(); p != m_table[index].end(); ++p){
  ...
  void print(std::ostream & ostr) {
	...
	void print(std::map <std::string, std::string> actors, iterator p){
		
		
		std::cout << "Printing " << (*p).second.size() << " result(s):" << std::endl;
		for(unsigned i = 0; i < (*p).second.size(); i++){
			std::cout << (*p).second[i][0][0] << std::endl;
			std::cout << (*p).second[i][1][0] << std::endl;
			std::cout << (*p).second[i][2][0] << std::endl;
			std::cout << (*p).second[i][3].size() << " ";
			for(unsigned j = 0; j< (*p).second[i][3].size(); j++){
				std::cout << (*p).second[i][3][j];
				if(j+1 != (*p).second[i][3].size()){
					std::cout <<" ";
				}
			}
			std::cout << std::endl;
			std::cout << (*p).second[i][4].size() << " ";
			for(unsigned j = 0; j < (*p).second[i][4].size(); j++){
				std::cout << actors[(*p).second[i][4][j]] << " (" << (*p).second[i][5][j] << ")";
				if(j+1 != (*p).second[i][4].size()){
					std::cout <<" ";
				}
			}
			std::cout << std::endl;
  ...
  void resize_table(unsigned int new_size) {
	...
	for(unsigned i=0; i<old_table.size();++i){
