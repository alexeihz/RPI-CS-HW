...
class BallDraw(object):
    ...
    def animate(self):
        ...
        while ball.some_inside(self.maxx,self.maxy) and not self.isstopped:
            # Move the ball
            self.draw_ball()
            ball.move()
            if(ball.some_inside(self.maxx,self.maxy)==False):
